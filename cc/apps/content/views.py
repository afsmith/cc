...
def assign_module(request):
    ...
    if request.user.get_profile().is_admin:
        ...
        users = CUser.objects.filter(groups__in=groups).distinct()
    else:
...
def find_files(request):
    ...
    for file in elements:
        ...
        entries.append({
            ...
            'is_removable': not _is_file_in_use_in_active_course(file.id) or request.user.get_profile().role == manage_models.UserProfile.ROLE_ADMIN,
            'usage': usage,
...
def save_manage_content(request):
    ...
    file.owner = CUser.objects.get(id=file_owner_id)
